...
void PmergeMe::sort(void) {
...

  {
    ...
    std::cout << "Time to process a range of " << _list.size()
              << " elements with std::list   : " << duration << " us"
              << std::endl;
  }
}
...
void PmergeMe::_sortArray(void) {
  ...
  // insert the second element of the pair in the correct position using binary
  // search
  for (std::vector<int>::const_iterator itr = pend.begin(); itr != pend.end();
       itr++) {
...
void PmergeMe::_sortList(void) {
  ...
  for (std::list<int>::const_iterator itr = pend.begin(); itr != pend.end();
       ++itr) {
    int index = _binarySearch(main, *itr);
